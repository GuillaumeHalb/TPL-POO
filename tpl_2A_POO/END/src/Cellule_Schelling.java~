...
public class Cellule_Schelling extends Cellule {
    ...
    /* Fonction calculant le nombre de voisins différents pour une maison situé en (i,j) */
    public int nb_voisins_differents(int i, int j, int taillex, int tailley) {
		...
		if (etat != 0) {
		    ...
		    if ((getEtatav(bot_line_i,right_raw_j) != etat) & (getEtatav(bot_line_i,right_raw_j) != 0)) 
				nb_voisins++;
		    if ((getEtatav(bot_line_i,left_raw_j) != etat) & (getEtatav(bot_line_i,left_raw_j) != 0))
				nb_voisins++;
		}
		return nb_voisins;
    }
